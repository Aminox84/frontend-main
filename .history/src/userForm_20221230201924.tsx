...
const UserForm = () => {
  ...
  const [idfinal,setIdFinal]=useState(0);


  const handleSubmitPartOne = ()=>{

    const p = axios.get(`http://localhost:3000/users`).then((res)=>{ setData(res.data) ; console.log(data)});

          
      var idFetch = Object.keys(data).length;
  ...
  const handleSubmitPartTwo=()=>{
    const p = axios.get(`http://localhost:3000/users`).then((res)=>{ setIdFinal(res.data.id.id)})
     
   axios.post(`http://localhost:3000/users/${idfinal}/profiles`, {
