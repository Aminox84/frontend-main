  ...
  return (
    <div style={{position:"relative",background:"white",border:"1px solid  black",padding:"2rem",margin:"1rem",borderRadius:".5rem",fontFamily:"Arial"}}>
      <form>
       ...
       {step}
       {!isFirstStep && <button className='btn btn-primary ml-2 d-flex justify-content-end' type="button" onClick={back}>Back</button>}
       <button type="button" className='btn btn-primary ml-2 d-flex justify-content-end' onClick={next}>{isLastStep ? "Finish" : "Next"}</button>
      </form>
    </div>
  );
}
